...
public class MiniChartComponent extends JComponent {
    ...
    @Override
    public @NonNull Dimension getMaximumSize() {
        return dimension;
    }
    ...
    protected void paintComponent(@NonNull Graphics g) {
        ...
        final int blockWidth = width / values.length;

        final float[] segments = new float[]{10_000_000f, 50_000_000, 100_000_000, 1_000_000_000};

        final SegmentedHeightTranslator ht = new SegmentedHeightTranslator(height, segments);

        g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        g2d.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_ON);
        g2d.setRenderingHint(RenderingHints.KEY_FRACTIONALMETRICS, RenderingHints.VALUE_FRACTIONALMETRICS_ON);

        final Color border = low.darker();
        ...
        for (int i = 0; i < values.length; i++) {
            final int rectHeight = ht.translate(values[i]);
        ...
        g2d.setColor(Color.BLACK);
        for (final float s : segments) {
            final int h = ht.translate(s);
            g2d.drawLine(0, height - h, width, height - h);
        }
    }

    /**
     * Demo entry point for testing the mini chart component.
     *
